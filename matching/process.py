...
def generate_match_matrix(
    mentor_list: List[Mentor], mentee_list: List[Mentee], rules: List[rl.Rule]
) -> List[List[Match]]:
    return [
...
def process_data(
    mentors: List[Mentor], mentees: List[Mentee], all_rules: List[List[rl.Rule]]
...
def conduct_matching_from_file(
    path_to_data: Path, rules: list[list[rl.Rule]]
) -> Tuple[List[Mentor], List[Mentee]]:
